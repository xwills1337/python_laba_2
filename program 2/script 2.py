...
first_year=int(df['Дата'][0][0]+df['Дата'][0][1]+df['Дата'][0][2]+df['Дата'][0][3])
last_year=int(df['Дата'][df.shape[0]-1][0]+df['Дата'][df.shape[0]-1][1]+df['Дата'][df.shape[0]-1][2]\
...
for first_year in range(first_year, last_year+1):
     ...
     data=lf['Дата'].iloc[0].replace('.', '')
     data=data+"_"+lf['Дата'].iloc[lf.shape[0]-1].replace('.', '')
     del lf['data_2']
